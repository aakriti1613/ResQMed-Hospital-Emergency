    ...
    <div className="min-h-full bg-[#0a0b0f] px-4 pt-8 pb-8 max-w-lg mx-auto w-full space-y-4">
      ...
      <div>
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-black uppercase tracking-widest text-sky-300/80">Hospital &amp; Care</span>
        </div>
        <h1 className="mt-1 text-2xl font-black text-white">Find the right doctor</h1>
        <p className="mt-1 text-xs text-white/40">
          Pick a department → see nearby hospitals → book a slot in under a minute.
        </p>
      </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          placeholder="Search specialty (e.g. Cardiology, Dental)"
      ...
      >
        ...
        <div className="relative flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-black uppercase tracking-widest text-emerald-300">Partner Hospital</span>
            ...
            <div className="text-[11px] text-white/45 truncate">
              {SHOWCASE_HOSPITAL.tagline} · All departments · Open 24×7
            </div>
          </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-2 px-0.5">
          <span className="text-[10px] font-black uppercase tracking-widest text-white/40">Browse by Department</span>
          <span className="text-[10px] text-white/30">{visible.length} of {DEPARTMENTS.length}</span>
        </div>
        {visible.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-white/10 bg-white/[0.02] p-8 text-center">
            ...
            <p className="text-sm font-bold text-white/50">No department found</p>
            <p className="text-xs text-white/30 mt-0.5">Try "General", "Heart" or "Skin"</p>
          </div>
        ) : (
              ...
              >
                ...
                >
                  ...
                  <div className="relative flex items-start gap-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-black text-white leading-tight truncate">{d.name}</div>
                      <div className="text-[10px] text-white/40 mt-0.5 truncate">{d.tagline}</div>
                    </div>
                  </div>
                </Link>
              </motion.div>
      ...
      <div className="rounded-2xl border border-white/[0.04] bg-white/[0.02] px-3 py-2.5 flex items-start gap-2">
        ...
        <p className="text-[10px] text-white/40 leading-relaxed">
          Hospitals near you are fetched live from Google Maps. Consultations are confirmed with our
          partner hospital &nbsp;<span className="text-white/70 font-semibold">Arogya Medicare</span>&nbsp;
          and saved to your Health Vault automatically.
        </p>
      </div>
    </div>
  );
